...
while true; do
    ...
    case "$STATE" in
        ...
        "DONE")
            ...
            # === REPRODUCIR SONIDO ===
            # Comprobamos si es el segundo exacto en el que terminó (NOW == LAST_ACT)
            if [ $(( NOW - LAST_ACT )) -eq 0 ]; then
                # Usa mpv para reproducir el archivo en segundo plano (&)
                # Cambia la ruta por la de tu archivo real
                (mpv /home/yemu/Música/car-alarm.mp3 --no-video --volume=100 > /dev/null 2>&1) &
            fi
            # ========================
